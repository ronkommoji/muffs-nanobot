...
describe("webui API helpers", () => {
  ...
  it("percent-encodes websocket keys when fetching webui-thread snapshot", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/sessions/websocket%3Achat-1/webui-thread?token=tok",
  ...
  it("serializes settings updates as a narrow query string", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/settings/update?model_preset=default&model=openrouter%2Ftest&provider=openrouter&timezone=Asia%2FShanghai&bot_name=nanobot&bot_icon=nb&tool_hint_max_length=120&token=tok",
      expect.objectContaining({ credentials: "same-origin" }),
    );
  });
  ...
  it("serializes provider settings updates without returning secrets", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/settings/provider/update?provider=openrouter&api_key=sk-or-test&api_base=https%3A%2F%2Fopenrouter.ai%2Fapi%2Fv1&token=tok",
      expect.objectContaining({ credentials: "same-origin" }),
    );
  });
  ...
  it("serializes web search settings updates", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/settings/web-search/update?provider=searxng&base_url=https%3A%2F%2Fsearch.example.com&max_results=8&timeout=45&use_jina_reader=false&token=tok",
      expect.objectContaining({ credentials: "same-origin" }),
    );
  });
  ...
  it("serializes image generation settings updates", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/settings/image-generation/update?enabled=true&provider=openrouter&model=openai%2Fgpt-5.4-image-2&default_aspect_ratio=16%3A9&default_image_size=2K&max_images_per_turn=3&token=tok",
      expect.objectContaining({ credentials: "same-origin" }),
    );
  });
  ...
  it("reads and writes persisted sidebar state", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/webui/sidebar-state?token=tok",
      expect.objectContaining({ credentials: "same-origin" }),
    );
